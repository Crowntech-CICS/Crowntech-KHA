...
<html lang="en">
    ...
    <body>
        ...
        <br>
        <h1 class="h1-bold" id="h1small">Choose Account to Edit</h1>
                ...
                <tbody>
                    <%
                        try {
                            ...
                            ps = con.prepareStatement("select * from users where residentclass = 'Admin' or residentclass = 'Staff' or residentclass = 'BOD';");
                            rs = ps.executeQuery();
                            while (rs.next()) {
                                    String fullname = rs.getString("lastname").trim() + ", " + rs.getString("firstname").trim() + " " + rs.getString("middleinitial").trim();
                                    String role = rs.getString("residentclass");
                                    String email = rs.getString("email");
                                    // display db contents
                    %>
                                    <tr onclick="window.location.href='${root}/accounts/edit/edit-admin.jsp?a=<%=rs.getString("userid")%>'">
                                        <td class="tableContentText"><%=fullname%></td>
                                        <td class="tableContentText"><%=role%></td>
